...
const tabMeta: Record<TabKey, { title: string; tooltip: string; details: string }> = {
  ...
  "Payment Stats": {
    title: "결제 통계",
    tooltip: "클래스타입별·강의별 실매출(쿠폰·포인트 차감 후)과 판매 건수를 기간별로 봅니다.",
    details:
      "CMS 결제 데이터를 클래스타입별 시계열과 강의별 순위로 분석합니다. 그래프·테이블 금액은 실매출(쿠폰·포인트 차감 후 실제 받은 금액) 기준입니다. 취소(환불)건은 제외됩니다. 정가 합(GMV)·총 할인액은 별도 KPI 로 참고용 제공하며 실매출 계산에는 사용하지 않습니다. 최고관리자 전용.",
  },
  "Project Settings": {
    title: "프로젝트 설정",
