/**
 * 브랜드 공식 URL 인용 집계 — 순수함수 모듈 (DB·Next 무의존).
 *
 * 목적: "인용 출처" 탭에서 내 사이트(브랜드 공식 채널) URL이 전체 기간 동안 한 번이라도
 * 인용된 개별 페이지 URL을 전수 노출하고, URL마다 그 URL을 인용한 질문(프롬프트)을
 * 드릴다운한다.
 *
 * 설계(계획 v2 §4·§8):
 ...
 */
function isBrandCitationKey(host: string, canonicalUrlKey: string, brandKeySet: Set<string>): boolean {
  if (SOCIAL_PLATFORM_DOMAINS.has(host)) {
...
/**
 * 펼쳐진 citation 행들을 브랜드 URL 단위로 집계 + keyset 페이지네이션.
 *
 * 집계 규칙 (계획 §5 하단):
...
export function aggregateBrandCitationUrls(
  ...
  type UrlAcc = {
    canonicalUrlKey: string;
    domain: string;
    // 대표 displayUrl 결정을 위해 (createdAtMs, displayUrl) 최소값 추적
    firstSeenMs: number;
  ...
  for (const row of rows) {
    ...
    if (!isBrandCitationKey(norm.host, norm.canonicalUrlKey, opts.brandKeySet)) {
      continue; // 비브랜드 URL 제외
    }
    ...
    // 대표 displayUrl / firstSeen — 더 이른 등장으로 갱신
  ...
  // UrlAcc → BrandCitationUrl 변환 + 정렬
  const all: BrandCitationUrl[] = [...urlMap.values()].map((acc) => {
  ...
  return { uniqueUrlCount, invalidCitationCount, urls: page, nextCursor };
}
...
/** URL 정렬 비교자: totalCount desc, canonicalUrlKey asc */
function compareUrl(a: BrandCitationUrl, b: BrandCitationUrl): number {
 ...
 */
function firstIndexAfterCursor(sorted: BrandCitationUrl[], cursor: UrlCursor): number {
...
/**
 * 특정 canonicalUrlKey 의 프롬프트를 전수 집계 + keyset 페이지네이션 (prompts endpoint 용).
 * rows 는 이미 해당 URL 로 좁혀졌다고 가정하지 않고, 이 함수 내에서 canonicalUrlKey 매칭으로 필터한다.
 */
export function aggregatePromptsForUrl(
  ...
  for (const row of rows) {
    ...
    if (!isBrandCitationKey(norm.host, norm.canonicalUrlKey, opts.brandKeySet)) continue;
...
/** 프롬프트 정렬(count desc, lastSeen desc, promptText asc) 기준 cursor 다음 인덱스.
