...
const tabMeta: Record<TabKey, { title: string; tooltip: string; details: string }> = {
  ...
  Abandoners: {
    ...
    //    키를 바꾸면 권한별 숨김 목록·저장된 화면 상태가 조용히 깨진다(types.ts 주석 참조).
    title: "관심 고객",
    tooltip:
      "관심을 보였지만 아직 결제·가입하지 않은 분을 '결제 전 이탈 / 가입 전 이탈 / 가입 전환' 세 가지로 나눠 봅니다.",
    details:
      // ⚠️ 이 설명문은 **정적 문자열**이라 기능이 바뀌면 스스로 낡는다(2026-07-21 · reviewer H2-b).
      //    두 군데가 실제로 거짓이 됐었다:
      //      ① "발송 가능 인원(마케팅 수신동의자)" — 탭 본문은 이미 "수신동의 표시"로 고쳤는데 여기만 옛 말.
      //      ② "가입 전 비로그인 조회는 연결할 수 없어" — 이번 작업이 바로 그걸 연결한다.
      //    ⚠️ **날짜를 문자열에 박지 말 것.** 언제부터 쌓인 기록인지는 탭 본문 패널이 서버값
      //       (firstSignupEventAt)으로 말한다 — 여기 박으면 스위치를 켠 날과 어긋나도 아무도 못 알아챈다.
      //    ⚠️ **권한 계층의 명칭을 적지 말 것**(2026-08-09 보안 검수 A-6). 이 문자열은 일반 관리자에게도
      //       전송되는 파일에 들어 있어, 개발자도구로 열면 그대로 읽힌다. "권한이 있는 계정만"으로 적는다
      //       (같은 기능의 두 화면 파일이 이미 그 표현을 쓰고 있다 — card-click-views.tsx ERROR_MESSAGES).
      "이 탭은 '결제 전 이탈 / 가입 전 이탈 / 가입 전환' 세 가지 하위 구분으로 나뉩니다. [결제 전 이탈] 과정 상세페이지를 본 뒤 결제하지 않은 회원을 '결제창 이탈 / 반복 조회 / 단순 조회 / 사각지대' 4개로 나눠 봅니다. 한 회원은 과정마다 정확히 한 칸에만 들어가므로 합계가 항상 맞습니다. 각 칸은 전체 인원과 '수신동의 표시가 있는 인원'을 함께 보여줍니다 — 표시가 있다는 것과 지금 광고를 보내도 된다는 것은 다르며, 가입만 한 회원의 표시 비율이 약 54%라 두 숫자는 크게 다릅니다. 맨 위 '결제창에도 안 갔고 결제 이력이 전혀 없는 회원'이 사장님 질문에 대한 직답입니다. 가입 전 비로그인 조회는 기능을 켠 날부터 쌓인 것만 반영되며 그 이전 것은 남아 있지 않아 되살릴 수 없습니다(언제부터인지는 화면 안에 함께 표시됩니다). '사각지대'는 지금 명단에서 조용히 빠지는 사람들로, 결제창 제외 기간을 조회 기간 이하로 낮추면 0이 됩니다. 이 화면은 이름·연락처를 그대로 표시하며 파일 내려받기는 제공하지 않습니다(조회 전용). [가입 전 이탈] 정규과정 상세페이지에서 맛보기 영상 카드나 전자책 카드를 눌렀지만 아직 회원가입하지 않은 사람을 강의별·책별로 봅니다. 누른 분 가운데 누를 때 이미 회원이셨던 분은 '이미 회원' 칸으로 따로 빼며, '아직 가입 안 함'과 전환율은 '누른 사람 − 이미 회원 = 비회원'을 기준으로 계산합니다. 같은 방문(브라우저 탭) 안에서 여러 번 누르신 것은 한 명으로 세지만, 다른 날 다시 오셔서 누르면 다른 사람으로 세어질 수 있어 비회원 인원은 실제보다 많게 나올 수 있습니다. 아래 합계 줄은 항목 줄을 더한 값이 아니라 사람을 한 번만 센 값입니다(한 분이 여러 개를 누르면 항목마다 들어갑니다). [가입 전환] 그 경로로 들어와 실제로 가입한 회원의 가입일시·이름·경로·본 항목·가입 방식을 봅니다. 한 분이 맛보기와 전자책을 둘 다 눌렀다면 두 줄로 나오므로 줄 수는 사람 수가 아니며, 탈퇴하신 분은 이 명단에서 빠지므로 같은 기간을 다시 조회하면 숫자가 줄어들 수 있습니다. 새 두 화면은 이름까지만 표시하며 연락처·이메일은 서버가 내려주지 않습니다. 항목 이름은 서버가 아는 값만 쓰고 확인되지 않은 항목은 '미상 항목'으로 구분해 적습니다. 클릭 기록·가입 기록은 수집 기능이 사이트에 반영된 뒤부터 쌓이므로 그 전까지는 0건이 정상이며, 언제부터 쌓인 기록인지와 가입 기록 수집이 살아 있는지는 화면 안에 함께 표시됩니다. 세 화면 모두 조회 전용이며 파일 내려받기는 제공하지 않습니다. 권한이 있는 계정만 볼 수 있습니다.",
  },
  "Project Settings": {
    title: "프로젝트 설정",
...
export function SovereignDashboard({ demoMode = false }: { demoMode?: boolean } = {}) {
  ...
  function calcVisibilityScore(
    ...
    let score = 66;
    if (firstPos < 200) score += 9;
    else if (firstPos < 500) score += 7;
    if (mentions >= 3) score += 7;
    else if (mentions >= 2) score += 3;

    if (sentiment === "positive") score += 9;
    else if (sentiment === "neutral") score += 8;

    if (isTopRanked) score += 8;
...
이제 총 ${competitorList.length}개 경쟁사를 분석하세요:`,
  ...
  async function handleResetManualResponses() {
    ...
    const manualCount = state.runs.filter((r) => r.auto !== true).length;
    if (manualCount === 0) {
      setMessage("삭제할 수동 실행 이력이 없습니다 (자동화 데이터는 이 버튼으로 지워지지 않습니다)");
      return;
    }

    if (!window.confirm(`수동으로 실행한 응답 ${manualCount}건만 삭제합니다.\n자동화 이력·프롬프트·설정은 유지됩니다. 계속하시겠습니까?`)) return;

    // 수동 실행 중인 요청만 취소 — 자동 스케줄 요청은 건드리지 않는다
    activeControllersRef.current.forEach((c) => c.abort());
    activeControllersRef.current.clear();
  ...
  function renderActiveTab() {
    ...
    // ⭐ 탭 키는 "Abandoners" 그대로 두고 **보이는 제목만** "관심 고객"으로 바꿨다(2026-08-09).
    //    키를 바꾸면 숨김 목록·저장된 activeTab 이 조용히 깨진다. 안에서 하위 구분 3개로 나뉜다.
    if (activeTab === "Abandoners") {
      return <InterestCustomersTab />;
    }
