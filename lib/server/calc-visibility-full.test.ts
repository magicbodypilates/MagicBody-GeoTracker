 ...
 *
 * calcVisibilityFull 은 룰 세트 레지스트리의 v14a 세트에 위임하는 얇은 어댑터다.
...
describe("수집 경로가 쓰는 세트 = v14a (위임 계약)", () => {
  const samples = [
  ...
  it("모든 표본에서 calcVisibilityFull == v14a 세트 직접 계산", () => {
...
describe("분기별 최대 경로 앵커(설계값)", () => {
  ...
  it("일반 최대(상단·반복3·긍정·topRanked): 66+9+7+9+8 = 99", () => {
    expect(
      score({
  ...
  it("일반 중단(200<=firstPos<500)·반복3·긍정·topRanked: 66+7+7+9+8 = 97", () => {
    expect(
      score({
        text:
          "가".repeat(210) +
  ...
  it("일반 mentions=0: 본문URL 55, 참고자료만 45, 둘 다면 본문 우선 55", () => {
    const base = { text: "요가 일반 답변(브랜드 미언급)", brandTerms: ["요가원"] };
    expect(score({ ...base, hasBodyUrl: true })).toBe(55);
    expect(score({ ...base, hasCitationOnly: true })).toBe(45);
    expect(score({ ...base, hasBodyUrl: true, hasCitationOnly: true })).toBe(55);
...
describe("경계·엣지", () => {
  ...
  it("일반 단일언급·상단·중립: 66 + firstPos 9 + neutral 8 = 83", () => {
