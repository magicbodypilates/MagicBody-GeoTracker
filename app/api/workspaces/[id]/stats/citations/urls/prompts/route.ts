/**
 * GET /api/workspaces/[id]/stats/citations/urls/prompts
 *
 * 특정 브랜드 URL(canonicalUrlKey)을 인용한 프롬프트(질문)를 전수 조회 (계획 A-1 조정).
 * URL 목록 응답의 inline top-N 을 넘어서는 나머지 프롬프트를 cursor 페이지네이션으로 끝까지.
 *
 * 쿼리:
 *   ?canonicalUrlKey=<key>   (필수 — urls 응답의 canonicalUrlKey)
 *   ?auto=true|false         (기본 true)
 *   ?branded=true|false      (기본 false)
 *   ?pageSize=50             (기본 50, 최대 200)
 *   ?cursor=<opaque>         (프롬프트 keyset cursor)
 *
 * 집계·정규화·매칭 규칙은 urls 라우트와 동일한 순수함수(aggregatePromptsForUrl)를 재사용.
 */
...
// env 파생 정수 상수 — urls 라우트와 동일 검증(계획 Info-2·B-1). STATEMENT_TIMEOUT_MS 는 raw SQL 인라인.
...
export async function GET(
  ...
  try {
    ...
    // 브랜드 호스트 superset 사전 필터 — urls 라우트와 동일. 특정 URL 의 프롬프트 전수도 브랜드 한정이므로
    // 펼침 단계에서 브랜드 후보로 좁혀 행 cap 이 브랜드 인용을 잘라내지 않게 한다. 정확 판정은 JS 유지.
    ...
    const res = aggregatePromptsForUrl(rows, canonicalUrlKey, { brandKeySet, pageSize, cursor });
    ...
    console.error("[/api/workspaces/:id/stats/citations/urls/prompts] 실패:", err);
